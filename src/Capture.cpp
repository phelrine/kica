...
void Capture::setFileName(const char *name)
{
...
bool Capture::start(XnUserID user_)
{
  ...
  sprintf(buf, "%s-%d.oni", filename, count);
  recorder->SetDestination(XN_RECORD_MEDIUM_FILE, buf);
  
  sprintf(buf, "%s-%d.csv", filename, count);
  out = fopen(buf, "w");
  printf("start\n");
  user = user_;
  frame = 0;
